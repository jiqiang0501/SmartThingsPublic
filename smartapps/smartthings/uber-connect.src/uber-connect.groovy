...
definition(
    ...
    author: "SmartThings",
    description: "Uber Service Manager",
    ...
    oauth: [displayName: "Uber", displayLink: "Uber"],
    singleInstance: true
) {
	appSetting "clientId"
...
def authPage()
{
    ...
    def description = "Uber Credentials Already Entered."
    
    def uninstallOption = false
    if (app.installationState == "COMPLETE")
       uninstallOption = true
    log.debug "1111"+getServerUrl()

	if(!state.life360AccessToken)
    {
	    log.debug "about to create access token"
		//createAccessToken()
        description = "Click to enter Uber Credentials."

		def redirectUrl = oauthInitUrl()
        //def redirectUrl = "https://www.baidu.com"
    
  	  	log.debug "RedirectURL = ${redirectUrl}"
        

		return dynamicPage(name: "Credentials", title: "Uber Auth", nextPage:"listCirclesPage", uninstall: uninstallOption, install:false) {
		    section {
    			href url:redirectUrl, style:"embedded", required:false, title:"Uber Auth", description:description
		    }
   	 	}
    }
    else
...
def receiveToken() {
    ...
    //log.debug "receiveToken: " + state.life360AccessToken
...
def oauthInitUrl()
{
    ...
    log.debug state.oauthInitState
	...
	return "https://login.uber.com.cn/oauth/v2/authorize?" + toQueryString(oauthParams)
}
